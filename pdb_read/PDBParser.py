...
class PDBParser(object):
    ...
    def __init__(self, PERMISSIVE=True, get_header=False,
        ...
        if structure_builder is not None:#####################
            ...
            self.structure_builder = StructureBuilder() ########得到一个StructureBuilder类
        self.header = None
        self.trailer = None
        self.line_counter = 0
    ...
    def get_structure(self, id, file):
        ...
        self.structure_builder.init_structure(id) # 给定的名字id
        if file[-2:] == 'gz':
            with gzip.open(file, 'r') as handle:   #######按照行读取pdb文件
                self._parse(handle.read().decode("utf-8").split('\n'))
        else:
            with open(file, 'r') as handle:   #######按照行读取pdb文件
    ...
    def _parse(self, header_coords_trailer):  #把PDB文件分为三部：头，坐标，尾部分
    ...
    def _parse_coordinates(self, coords_trailer):
        ...
        for i in range(0, len(coords_trailer)):
            ...
            if record_type == "ATOM  " or record_type == "HETATM":
                ...
                #if occupancy is not None and occupancy < 0: ###########################？？？？？？？？直接写occupancy < 0
